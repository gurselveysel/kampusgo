...
export default function MedicalAiStudioPage() {
  ...
  return (
    <main className={styles.page}>
      ...
      <div className={styles.banner}><b>KONTROLLÜ PİLOT</b><span>Sentetik hasta · Uzman onay referansı zorunlu · Klinik karar desteği değildir</span></div>
      ...
      {session === "authenticated" ? <section className={styles.workspace}>
        <form className={styles.form} onSubmit={createJob}>
          ...
          <button className={styles.primary} disabled={submitting || !rights || !synthetic || approval.trim().length < 6}>{submitting ? "Üretim sürüyor…" : "arXivisual sahnesini üret"}</button>
        </form>

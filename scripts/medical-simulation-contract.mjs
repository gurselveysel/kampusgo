...
const clinicalExperience = read("app/medikal-simulasyon/ClinicalSimulationExperience.tsx");
const clinicalEngine = read("app/medikal-simulasyon/clinical-engine.js");
const clinicalCss = read("app/medikal-simulasyon/clinical-simulation.module.css");
...
assert.match(page, /ClinicalSimulationExperience/);
assert.match(page, /Uçtan uca klinik vaka/);

assert.match(clinicalEngine, /teys-stemi-engine\/2\.0\.0/);
assert.match(clinicalEngine, /previousHash/);
assert.match(clinicalEngine, /nextHash/);
assert.match(clinicalEngine, /replaySession/);
assert.match(clinicalEngine, /electricalInstability/);
assert.match(clinicalEngine, /PDE5_NITRATE/);
assert.match(clinicalEngine, /state\.phase = "vf"/);
assert.match(clinicalEngine, /state\.phase = "rosc"/);
assert.match(clinicalEngine, /practiceLevel: 4/);
assert.match(clinicalEngine, /officialPlacementStatus: "DOĞRULANMADI"/);
assert.match(clinicalExperience, /Hasta görüşmesi/);
assert.match(clinicalExperience, /Fizik muayene/);
assert.match(clinicalExperience, /Tetkik ve görüntüleme/);
assert.match(clinicalExperience, /ADMINISTER_MEDICATION/);
assert.match(clinicalExperience, /Deterministik replay/);
assert.match(clinicalExperience, /EĞİTİCİ ANALİTİĞİ/);
assert.match(clinicalExperience, /onError=\{\(\) => setFailed\(true\)\}/);
assert.match(clinicalCss, /prefers-reduced-motion/);
assert.match(clinicalCss, /@media \(max-width: 480px\)/);
assert.match(clinicalCss, /content-visibility: auto/);

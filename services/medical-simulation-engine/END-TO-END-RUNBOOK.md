...
`.github/workflows/medical-simulation-arxivisual-seed.yml` gerçek Docker image'ını kurar, FastAPI job endpoint'ine `presets/vf-rosc.json` gönderir, işi tamamlanana kadar izler ve üretilen MP4 ile manifesti 14 günlük CI kanıt artefaktı olarak yükler. Dal CI tarafından değiştirilmez.

Önizlemede gösterilen sürümlü yerel kanıt `assets/medical-simulation/` altında tutulur ve `scripts/build-public.mjs` tarafından `public/medical-simulation/manim/` yoluna kopyalanır. Manifest, bu kanıt için uzman onay durumunu açıkça `DOĞRULANMADI` olarak kaydeder.

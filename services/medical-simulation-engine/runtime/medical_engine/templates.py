...
def deterministic_scene_code(request: MedicalSceneRequest) -> tuple[str, str]:
    ...
    action = _label(request.learner_action.label, 54)
    critical = _label(request.critical_signal, 62)
    question = _label(request.debrief_question, 72)
    before_signs = _label(", ".join(before.visible_signs[:3]) or "Başlangıç bulguları", 54)
    after_signs = _label(", ".join(after.visible_signs[:3]) or "Müdahale sonrası bulgular", 54)
...
class {class_name}(Scene):
    def construct(self):
        ...
        before_label = Text({_py(before_signs)}, font_size=18, color=AMBER)
        before_label.next_to(patient_panel.get_top(), DOWN, buff=0.35)
        action_label = Text({_py(action)}, font_size=20, color=WHITE, weight=BOLD)
        ...
        footer = Text({_py("Uzman onayı: " + _label(request.expert_approval_reference, 62))}, font_size=13, color=MUTED)
        ...
        signal_box = RoundedRectangle(width=11.9, height=0.72, corner_radius=0.15, color=RED, fill_color="#3A1820", fill_opacity=0.95)
        signal_box.to_edge(DOWN, buff=0.72)
        signal = Text({_py("KRİTİK SİNYAL · " + critical)}, font_size=19, color=WHITE, weight=BOLD)
        ...
        debrief = Text({_py(question)}, font_size=19, color=WHITE, weight=BOLD)
        debrief.set_max_width(11.2).move_to(debrief_box.get_center())
